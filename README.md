...
* Apache & mod_rewrite / Nginx / OpenLiteSpeed
...
10) php artisan make-admin {email} # To add new admin user
````
...
1. In `.env`, set `ASAAS_ENV` (sandbox/production), `ASAAS_API_KEY`, and `ASAAS_WEBHOOK_TOKEN`.
2. Configure the webhook in your Asaas dashboard:
   - URL: `https://your-domain.com/webhooks/asaas`
   - Token: Use the same value as `ASAAS_WEBHOOK_TOKEN` in the `asaas-access-token` header.
   - Events: Select `PAYMENT_RECEIVED`, `PAYMENT_CONFIRMED`, and `PAYMENT_OVERDUE`.
