...
<div class="input-group mb-3 mt-3 deposit-form-wrapper">
    <div class="input-group-prepend">
        <span class="input-group-text"
            id="amount-label">@include('elements.icon', ['icon' => 'cash-outline', 'variant' => 'medium'])</span>
    </div>
...
<div>
    <div class="payment-method">
        ...
        @if(getSetting('payments.nowpayments_api_key'))
            <div class="custom-control custom-radio mb-1">
                ...
                <label class="custom-control-label stepTooltip" for="customRadio5"
                    title="">{{__("NowPayments Crypto")}}</label>
            </div>
        @endif
        ...
        @endif
        @if(getSetting('payments.razorpay_api_key') && getSetting('payments.razorpay_api_secret'))
            <div class="custom-control custom-radio mb-1">
                <input type="radio" id="customRadio10" name="payment-radio-option" class="custom-control-input"
                    value="payment-razorpay">
                <label class="custom-control-label stepTooltip" for="customRadio10" title="">{{__("Razorpay")}}</label>
            </div>
        @endif
        @if(config('asaas.api_key'))
            <div class="custom-control custom-radio mb-1">
                <input type="radio" id="asaas_pix" name="payment-radio-option" class="custom-control-input"
                    value="payment-asaas_pix">
                <label class="custom-control-label stepTooltip" for="asaas_pix" title="">{{__("Asaas PIX")}}</label>
            </div>
            <div class="custom-control custom-radio mb-1">
                <input type="radio" id="asaas_boleto" name="payment-radio-option" class="custom-control-input"
                    value="payment-asaas_boleto">
                <label class="custom-control-label stepTooltip" for="asaas_boleto" title="">{{__("Asaas Boleto")}}</label>
            </div>
        @endif
            ...
            <div class="manual-details d-none">
                ...
                @if(getSetting('payments.offline_payments_iban'))
                    <div class="alert alert-primary text-white font-weight-bold" role="alert">
                        <p class="mb-0">
                            {{__('Once confirmed, your credit will be available and you will be notified via email.')}}</p>
                        <ul class="mt-2 mb-2">
                            <li>{{__('IBAN')}}: <span
                                    class="font-weight-bold">{{getSetting('payments.offline_payments_iban')}}</span></li>
                            <li>{{__('BIC/SWIFT')}}: <span
                                    class="font-weight-bold">{{getSetting('payments.offline_payments_swift')}}</span></li>
                            <li>{{__('Bank name')}}: <span
                                    class="font-weight-bold">{{getSetting('payments.offline_payments_bank_name')}}</span></li>
                            <li>{{__('Account owner')}}: <span
                                    class="font-weight-bold">{{getSetting('payments.offline_payments_owner')}}</span></li>
                            <li>{{__('Account number')}}: <span
                                    class="font-weight-bold">{{getSetting('payments.offline_payments_account_number')}}</span>
                            </li>
                            <li>{{__('Routing number')}}: <span
                                    class="font-weight-bold">{{getSetting('payments.offline_payments_routing_number')}}</span>
                            </li>
                        </ul>
                    </div>
                @endif
                ...
                <p class="mb-1 mt-2">
                    {{__("Please attach clear photos with one the following: check, money order or bank transfer.")}}</p>
                <div class="dropzone-previews dropzone manual-payment-uploader w-100 ppl-0 pr-0 pt-1 pb-1 border rounded">
                </div>
                <small class="form-text text-muted mb-2">{{__("Allowed file types")}}:
                    {{str_replace(',', ', ', AttachmentHelper::filterExtensions('manualPayments'))}}.</small>
                <div class="text-danger invalid-files d-none">
    ...
    <button class="btn btn-primary btn-block rounded mr-0 mt-4 deposit-continue-btn"
