...
var StoriesProfile = {
    ...
    renderHighlightsRow: function (stories) {
        ...
        (stories || []).forEach((bubble, bubbleIndex) => {
            ...
            var first = items[0];
            var cover = bubble.photo || "";

            if (first) {
                // 1) Text-stories -> avatar
                if (first.type === "text") {
                    cover = bubble.photo || "";
                }

                // 2) Videos -> thumb if has_thumbnail else avatar
                else if (first.type === "video") {
                    var hasThumbV = !!first.has_thumbnail;
                    if (hasThumbV && first.preview) {
                        cover = String(first.preview);
                    } else {
                        cover = bubble.photo || "";
                    }
                }
                else if (first.type === "image") {
                    var hasThumbI = !!first.has_thumbnail;
                    if (hasThumbI && first.preview) {
                        cover = String(first.preview);
                    } else if (first.src) {
                        cover = String(first.src);
                    } else {
                        cover = bubble.photo || "";
                    }
                }
